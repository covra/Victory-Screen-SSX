...
local function UpdatePanelForPlayer(panel, player)
   ...
   -- killsValueLabel.text = tostring(player.kills)
    ...
    --Task.Wait(3)
    --killsValueLabel.text = tostring(player:GetResource("fameR"))
    --deathsValueLabel.text = tostring(player:GetResource("coinsR"))
	-----------------------------------------------------------------------
...
---------------------SSx-----------------------------------------
function onUpdateRes (player, res, resNum)
	if res == "fameR" then 
		killsValueLabel.text = tostring(resNum)
	elseif res == "coinsR" then 
		deathsValueLabel.text = tostring(resNum)
	 end
end
-------------------------------------------------------------------
...
Events.Connect('updateVS', onUpdateRes)
