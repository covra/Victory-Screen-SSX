...
function API.CalculateWinners(victoryScreen,winnerSortType, winnerSortResource)
	...
	if victoryScreen:GetCustomProperty("UseTeams") then
		...
		winners = _G.currentRacers({includeTeams = WiningTeam})
	
	else
		winners = _G.currentRacers
	end
...
function API.OnPlayerTeleported(victoryScreen, player, duration, respawnOnDeactivate)
	...
	-----------------------------------------SSx------------------------------
	--player:Spawn()
	--player:Spawn({spawnKey = tostring(player.serverUserData.puesto)})
	player:Spawn({spawnKey = "lobby"})----ADDED CVS
	--------------------------------------------------------------------------------
	Task.Spawn(function()
